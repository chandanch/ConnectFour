...
public class Connect4View extends View {
	...
	private void initGUI(Context context){
		paintGrid.setColor(Color.rgb(255, 255, 153));
		paintGrid.setTextSize(18);
		paintCircleHuman.setColor(Color.rgb(7, 78, 243));
		paintCircleComputer.setColor(Color.rgb(0, 255, 0));
    ...
    private void drawCalculationMessage(Canvas canvas, GameImp game2) {
		int count = game2.getCalculatedCounts();
		int next = game2.getNextComputerStepShow();
		if(count != -1 && next !=-1){
			String message = "Computer next: "+next +", calculated nodes: "+ count;
			//canvas.drawText(message, 10, controlHeight-30, paintGrid);
		}
    ...
    private void drawWin(Canvas canvas){
		...
		if (win){
			...
			paint.setColor(Color.rgb(178, 102, 255));
